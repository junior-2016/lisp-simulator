//
// Created by junior on 19-5-14.
//

#ifndef LISP_SIMULATOR_PROCEDURE_H
#define LISP_SIMULATOR_PROCEDURE_H
...
namespace lisp {
    ...
    private:
        ...
        Env::handle env;
    public:
        Procedure(Ast::ptr body, const std::vector<string_t> &args_names, Env::handle env) : Function() {
        ...
        Value operator()(const std::vector<Value> &args) override {
            return eval(std::move(this->function_body),
                        make_ptr<Env>(this->function_args_names, args, this->env));
        }

        ~Procedure() override = default;
    };
}
#endif //LISP_SIMULATOR_PROCEDURE_H
